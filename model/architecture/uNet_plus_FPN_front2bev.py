...
def joiner(encoder_layers, thetas, kernel_size=(3,3), activation=tf.nn.relu, batch_norm=True):
    ...
    warped = t = SpatialTransformer(shape, shape, theta_init=thetas, theta_const=True)(encoder_layers[0])
    ...
    warped = t = SpatialTransformer(shape, shape, theta_init=thetas, theta_const=True)(encoder_layers[1])
    # = Conv2D(filters=filters2, kernel_size=kernel_size, padding="same", activation=activation)(t)
    #t = BatchNormalization()(t) if batch_norm else t
    #t = Conv2D(filters=filters2, kernel_size=kernel_size, padding="same", activation=activation)(t)
    #t = warped_layers[0] = BatchNormalization()(t) if batch_norm else t
    #warped = t = BatchNormalization()(t) if batch_norm else t

    nonwarped = t = encoder_layers[1]
    #t = Conv2D(filters=filters2, kernel_size=kernel_size, padding="same", activation=activation)(t)
    #t = BatchNormalization()(t) if batch_norm else t
    #t = Conv2D(filters=filters2, kernel_size=kernel_size, padding="same", activation=activation)(t)
    #nonwarped = t = BatchNormalization()(t) if batch_norm else t
    ...
    warped = t = SpatialTransformer(shape, shape, theta_init=thetas, theta_const=True)(encoder_layers[2])
    ...
    warped = t = SpatialTransformer(shape, shape, theta_init=thetas, theta_const=True)(encoder_layers[3])
    ...
    warped = t = SpatialTransformer(shape, shape, theta_init=thetas, theta_const=True)(encoder_layers[4])
